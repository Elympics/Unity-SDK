...
namespace Elympics.Weaver
{
    ...
    public class Log
    {
        ...
        private string FormatLabel(string message, string fileName, int lineNumber, MessageType logType)
        {
            ...
            {
                MessageType.Warning => string.Format("<color=yellow>[{0}:{1}]: {2}</color>", fileName, lineNumber, message),
                MessageType.Error => string.Format("<color=red>[{0}:{1}]: {2}</color>", fileName, lineNumber, message),
                _ => string.Format("[{0}:{1}]: {2}", fileName, lineNumber, message),
            };
        }
