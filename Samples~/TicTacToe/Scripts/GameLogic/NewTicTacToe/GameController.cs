...
namespace GameLogic.NewTicTacToe
{
    ...
    {
        [SerializeField] private GameStateObject gameState;
        ...
        {
            for (var i = 0; i < gameState.fields.Count; i++)
            {
        ...
        {
            var index = gameState.fields.FindIndex(field.Equals);
            var coordinates = IndexToCoordinates(index);
            var length = LineLength(coordinates, playerSide);
            if (length == 3)
                GameOver(playerSide);
            _occupiedFields++;
        ...
        private void GameOver(PlayerSide winner)
        {
            ...
            if (Elympics.IsServer)
                _ = StartCoroutine(EndGameOnServer(winner));
        }
        ...
        private Vector2Int IndexToCoordinates(int index) => new(index % 3, index / 3);
        ...
        private int LineLength(Vector2Int coordinates, PlayerSide playerSide)
        {
            ...
            for (var directionIndex = 0; directionIndex < 4; directionIndex++)
            {
                ...
                var leftLength = LineLengthInDirection(coordinates, playerSide, delta);
                var rightLength = LineLengthInDirection(coordinates, playerSide, -delta);
