...
namespace Elympics.Tests
{
	...
	public class TestCloudPing
	{
		...
		public IEnumerator TestNonExistingRegion() => UniTask.ToCoroutine(async () =>
		{
			...
			try
			{
				await ElympicsCloudPing.ChooseClosestRegion(new[] { "non-existing-region" });
			}
			catch (Exception e)
			{
				exc = e;
		...
		public IEnumerator TestLatencyDataZeroRegions() => UniTask.ToCoroutine(async () =>
		{
			...
			try
			{
				await ElympicsCloudPing.GetLatencyDataForRegions(Array.Empty<string>());
			}
			catch (Exception e)
			{
				exc = e;
		...
		public IEnumerator TestLatencyDataNullRegions() => UniTask.ToCoroutine(async () =>
		{
			...
			try
			{
				await ElympicsCloudPing.GetLatencyDataForRegions(null);
			}
			catch (Exception e)
			{
				exc = e;
		...
		public IEnumerator TestLatencyDataNonExistingRegion() => UniTask.ToCoroutine(async () =>
		{
			...
			try
			{
				await ElympicsCloudPing.GetLatencyDataForRegions(new[] { "non-existing-region" });
			}
			catch (Exception e)
			{
				exc = e;
